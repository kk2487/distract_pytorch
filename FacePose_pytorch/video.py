...
def main(args):
	...
	videoWriter = cv2.VideoWriter("./video/result.avi",cv2.VideoWriter_fourcc('X','V','I','D'),fps,(720,1280))
	...
	while success:
		...
		for (x,y) in pre_landmark.astype(np.float32):
			point_dict[f'{i}'] = [x,y]
			cv2.circle(cropped,(int(get_num(point_dict, i, 0)),int(get_num(point_dict, i, 1))), 1, (255, 0, 0), -1)
			i += 1
